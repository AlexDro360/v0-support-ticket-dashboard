...
import { Package, Laptop, Printer, Wifi, Calendar, MapPin, Server, HardDrive, Cpu, Monitor, Zap, Database } from 'lucide-react'
...
function EstadoBadge({ estado }: { estado: string }) {
  const estadoConfig = {
    Activo: { variant: 'default', class: 'bg-green-100 text-green-800' },
    Inactivo: { variant: 'secondary', class: 'bg-gray-100 text-gray-800' },
    Mantenimiento: { variant: 'secondary', class: 'bg-blue-100 text-blue-800' },
    Baja: { variant: 'destructive', class: 'bg-red-100 text-red-800' },
  } as const

  const config = estadoConfig[estado as keyof typeof estadoConfig] || estadoConfig.Inactivo
...
function TipoEquipoBadge({ tipo }: { tipo: string }) {
  ...
  const config = tipoConfig[tipo as keyof typeof tipoConfig] || tipoConfig.computo
...
function CampoDetalle({ icono, label, valor }: { icono: React.ReactNode; label: string; valor: string }) {
  return (
        ...
        <Card className="border-0 shadow-sm">
          ...
          <CardContent className="p-4 sm:p-6">
            <div className="grid gap-6 md:grid-cols-2">
              ...
              />
              <CampoDetalle
                icono={<Server className="h-4 w-4 text-muted-foreground" />}
                label="RAM"
                valor={`${equipo.ramGB} GB`}
              />
              <CampoDetalle
                icono={<Monitor className="h-4 w-4 text-muted-foreground" />}
